			...
			<div className='absolute inset-0 bg-black text-white min-h-screen'>
				<div className='p-8 grid gap-4 bg-black'>
					<div>
						...
						<button onClick={() => setShowAllPhotos(false)} className='fixed right-8 top-12 flex gap-1 rounded-2xl p-2 shadow shadow-black bg-white text-black'>
					...
					{place?.photos?.length > 0 && place.photos.map(photo => (
						<div>
							<img className='h-[600px] w-[600px] overflow-hidden object-fit mx-auto my-4' src={'http://localhost:4000/uploads/' + photo} alt="" />
						</div>
					))}
				</div>
			</div>
    ...
    return (
        <div>
            <div className='relative'>
                ...
                <button onClick={() => setShowAllPhotos(true)} className='flex gap-1 absolute bottom-2 right-2 py-2 px-4 bg-white rounded-lg border border-black '>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
